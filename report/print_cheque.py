...
class print_check(models.AbstractModel): 
    ...
    def get_partner_name(self,obj,p_text, isCommercial):                       
        isComm = isCommercial and obj.partner_id.commercial_name
        if p_text and obj.partner_text:
            if p_text == 'prefix' :
                if isComm:
                    return obj.partner_text + ' ' + obj.partner_id.commercial_name
                else:
                    return obj.partner_text + ' ' + obj.partner_id.name
            else:
                if isComm:
                    return obj.partner_id.commercial_name + ' ' + obj.partner_text
                else:
                    return obj.partner_id.name + ' ' + obj.partner_text

        if isComm:
            return obj.partner_id.commercial_name
        else:
            return obj.partner_id.name
        
    
    def get_partner_commercial_name(self,obj,p_text):            
        if p_text and obj.partner_text:
            if p_text == 'prefix' :
                return obj.partner_text + ' ' + obj.partner_id.commercial_name
            else:
    ...
    def amount_word(self, obj):
        ...
        if float(amt_lst[1]) > 0:
            lst.append(' con ' + amt_lst[1].ljust(2, '0') + ' / '+str(100))
        else:
            lst.append(' con 00 / ' + str(100))
    ...
    def _get_report_values(self, docids, data=None):
        ...
        return {
            ...
            'ref_ncf': self.get_ref_ncf(docs.ref),
        }
